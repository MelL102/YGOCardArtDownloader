...
def getCardImageURL(response_json):
    return response_json['card_images'][0]['image_url_cropped']
...
def downloadCardImage(response_json):  
    ...
    for x in response_json['data']:
        try:
            image_source = getCardImageURL(x)
            card_name = getCardName(x)
            urllib.request.urlretrieve(image_source,card_name+'.jpg')
        except: print("Image for: "+ card_name +" could not be found")
        time.sleep(0.1)
